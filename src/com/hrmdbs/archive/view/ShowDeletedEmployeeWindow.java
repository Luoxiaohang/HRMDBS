...
public class ShowDeletedEmployeeWindow extends JFrame implements MouseListener {
	...
	public ShowDeletedEmployeeWindow() {
		initWindow();
	}

	public ShowDeletedEmployeeWindow(ArrayList<Employee> es) {
		this.es = es;
		initData();
		initWindow();
	}
	...
	public static void main(String[] args) {
		ShowDeletedEmployeeWindow checkWindow = new ShowDeletedEmployeeWindow();
		checkWindow.initWindow();
	}

	public void mouseClicked(MouseEvent e) {
		if (e.getButton() == MouseEvent.BUTTON1) {
			String hrid = info[jt.getSelectedRow()][0];
			if (null != hrid && hrid.length() != 0) {
				int res;
				res = JOptionPane.showConfirmDialog(null, "你确定要恢复当前档案资料么？",
						"恢复档案资料", JOptionPane.YES_NO_OPTION,
						JOptionPane.QUESTION_MESSAGE);
				if (res == JOptionPane.YES_OPTION) {
					boolean success = EmployeeService.recoverEmployee(es.get(
							jt.getSelectedRow()).getId());
					if (success) {
						JOptionPane.showMessageDialog(null, "恢复成功", null,
								JOptionPane.INFORMATION_MESSAGE);
					} else {
						JOptionPane.showMessageDialog(null, "恢复失败！", null,
