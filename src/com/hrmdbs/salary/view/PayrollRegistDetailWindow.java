...
public class PayrollRegistDetailWindow extends JFrame {
	ArrayList<Employee> sris = null;
	String[][] info = new String[10][12];
	private JTextField textField;
	private JTable jt;
	private JButton jb_commit;

	public PayrollRegistDetailWindow(ArrayList<Employee> arrayList) {
		this.sris = arrayList;
		initDate();
		initWindow();
		initListener();
	...
	public static void main(String[] args) {
		new PayrollRegistDetailWindow(null);
	}

	private void initDate() {
		int i = 0;
		for (Employee em : sris) {
			info[i][0] = em.getHrid();
			info[i][1] = em.getName();
			ArrayList<SalaryStand> sss = SalaryService.getSlaryStand(em
					.getSalaryStand().getSalaryStandName().trim());
		...
		jb_commit.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < sris.size(); i++) {
					...
					bonus.setEmployee(sris.get(i));
					bonus.setRechecked(false);
					bonus.setBonus(Float.valueOf(String.valueOf(jt.getValueAt(
							i, 10))));
					bonus.setDeductedBonus(Float.valueOf(String.valueOf(jt
							.getValueAt(i, 11))));
					bonuss.add(bonus);
				}
				if (SalaryService.saveBonus(bonuss)) {
					JOptionPane.showMessageDialog(null, "登记成功！", null,
							JOptionPane.INFORMATION_MESSAGE);
				} else {
					JOptionPane.showMessageDialog(null, "登记失败！", null,
							JOptionPane.INFORMATION_MESSAGE);
				}
